...
# 构造绝对路径
coconame = os.path.join(script_dir, "coco.names")
yolovcofig = os.path.join(script_dir, "yolov3.cfg")
yolovwieght = os.path.join(script_dir, "yolov3.weights")
# print(coconame)
...
def shiTu(image_list, keyword):
    ...
    for r_image in image_list:
        ...
        classes = open(coconame, 'rt').read().strip().split("\n")
        # ===========模型初始化、推理===========
        net = cv2.dnn.readNetFromDarknet(yolovcofig,yolovwieght)
